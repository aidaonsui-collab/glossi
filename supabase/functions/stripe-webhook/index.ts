...
// Supabase secrets are required at boot. Stripe webhook secrets are
// checked lazily in collectSecrets() — we accept either of two
// (account events vs Connect events), and the request handler rejects
// if neither is present, so we don't want to crash the module on a
// missing secret if the other one IS configured.
