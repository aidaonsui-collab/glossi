...
const PROVIDER_KEY = { apple: 'apple', google: 'google', fb: 'facebook' };

export default function SocialSignIn({ redirectTo = '/quotes', compact = false }) {
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: compact ? '4px 0' : '6px 0', fontFamily: type.body, fontSize: 10, color: p.inkMuted, fontWeight: 700, letterSpacing: '0.14em' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
        {PROVIDERS.map(s => (
          <button key={s.id} type="button" onClick={() => handle(s)} style={{
            padding: compact ? '10px 10px' : '12px 14px', borderRadius: 12,
