...
export default function QuoteDetail() {
  ...
  const onAccept = async bid => {
    const name = bid.businesses?.name || (lang === 'es' ? 'este salón' : 'this salon');
    const msg = lang === 'es'
      ? `¿Pagar ${fmtPrice(bid.price_cents)} para reservar con ${name}? Glossi retiene los fondos y los libera después de tu cita.`
      : `Pay ${fmtPrice(bid.price_cents)} to book with ${name}? Glossi holds the funds and releases them after your appointment.`;
  ...
  const submitCounter = async () => {
    ...
    if (!Number.isFinite(price) || price <= 0) {
      toast(t('Enter a valid counter price.', 'Ingresa un precio de contraoferta válido.'), { tone: 'warn' });
      return;
    }
    if (price >= ask) {
      toast(lang === 'es' ? `La contraoferta debe ser menor a $${ask}.` : `Counter must be below $${ask}.`, { tone: 'warn' });
      return;
    }
    ...
    toast(lang === 'es' ? `Contraoferta enviada · $${price}.` : `Counter sent · $${price}.`, { tone: 'success' });
  ...
  if (!quote) {
    return (
      <CustomerLayout active="quotes" mobileTitle={t('Request', 'Solicitud')}>
        <div style={{ padding: '40px 30px', color: p.inkMuted, fontSize: 14 }}>
          {isSupabaseConfigured ? t('Loading…', 'Cargando…') : t('Sign in with a Supabase-backed account to view real quotes.', 'Inicia sesión con una cuenta de Supabase para ver cotizaciones reales.')}
        </div>
      </CustomerLayout>
    );
  }
  ...
  return (
    <CustomerLayout active="quotes" mobileTitle={t('Request', 'Solicitud')}>
      <div style={{ padding: isPhone ? '20px 18px 60px' : '34px 40px 60px', maxWidth: 720 }}>
        <Link to="/quotes" style={{ fontSize: 12, color: p.inkMuted, textDecoration: 'none', fontWeight: 600 }}>{t('← All requests', '← Todas las solicitudes')}</Link>
        ...
        <div style={{ marginTop: 28, fontSize: 10.5, fontWeight: 700, letterSpacing: '0.18em', color: p.inkMuted }}>
          {t('BIDS', 'COTIZACIONES')} · {bids.length}
        </div>

        {bidsLoading && bids.length === 0 ? (
          <div style={{ padding: 30, color: p.inkMuted, fontSize: 14 }}>{t('Loading bids…', 'Cargando cotizaciones…')}</div>
        ) : bids.length === 0 ? (
          <div style={{ marginTop: 14, padding: 24, background: p.surface, borderRadius: 14, border: `0.5px dashed ${p.line}`, textAlign: 'center', color: p.inkSoft, fontSize: 14, lineHeight: 1.5 }}>
            {t("No bids yet. Salons usually respond within an hour. We'll send you a notification.", 'Aún no hay cotizaciones. Los salones suelen responder en menos de una hora. Te enviaremos una notificación.')}
          </div>
        ) : (
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ fontFamily: type.display, fontStyle: 'italic', fontSize: 18, fontWeight: type.displayWeight, color: p.ink, letterSpacing: '-0.01em' }}>{b.businesses?.name || t('Salon', 'Salón')}</span>
                        {b.businesses?.verified && <span style={{ fontSize: 10, color: p.accent, fontWeight: 700, letterSpacing: '0.1em' }}>{t('VERIFIED', 'VERIFICADO')}</span>}
                      </div>
                      <div style={{ fontSize: 11.5, color: p.inkMuted, marginTop: 2 }}>
                        {b.businesses?.city || ''}{b.businesses?.price_tier ? ` · ${b.businesses.price_tier}` : ''} · {fmtAgo(b.created_at, lang)}
                      </div>
                      {b.message && (
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: p.inkMuted, marginTop: 2 }}>{fmtDur(b.estimated_duration, lang)}</div>
                    </div>
                  </div>
                  {isCountered && (
                    <div style={{ marginTop: 12, padding: '10px 12px', background: p.bg, borderRadius: 10, border: `0.5px solid ${p.accent}`, fontSize: 12.5, color: p.inkSoft, lineHeight: 1.5 }}>
                      <span style={{ color: p.accent, fontWeight: 700, letterSpacing: '0.1em', fontSize: 10.5 }}>{t('YOUR COUNTER', 'TU CONTRAOFERTA')} · {fmtPrice(b.counter_offer_cents)}</span>
                      {b.counter_message && <div style={{ marginTop: 4, fontStyle: 'italic' }}>"{b.counter_message}"</div>}
                      <div style={{ marginTop: 4, color: p.inkMuted, fontSize: 11.5 }}>{lang === 'es' ? `Enviada ${fmtAgo(b.counter_at, lang)} · esperando a ${b.businesses?.name || 'el salón'}` : `Sent ${fmtAgo(b.counter_at, lang)} · waiting for ${b.businesses?.name || 'salon'}`}</div>
                    </div>
                  )}
                  {isActionable && (
                    <div style={{ marginTop: 12, paddingTop: 12, borderTop: `0.5px solid ${p.line}`, display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                      ...
                      {isCountered && (
                        <button onClick={() => openCounter(b)} style={{
                          ...
                          {t('Edit counter', 'Editar contraoferta')}
                        </button>
                      )}
                      <button onClick={() => onAccept(b)} disabled={accepting === b.id || isCountered} style={{
                        ...
                        {accepting === b.id ? t('Booking…', 'Reservando…') : (lang === 'es' ? `Aceptar · ${fmtPrice(b.price_cents)}` : `Accept · ${fmtPrice(b.price_cents)}`)}
                      </button>
                    </div>
                  )}
                  {isAccepted && (
                    <div style={{ marginTop: 12, paddingTop: 12, borderTop: `0.5px solid ${p.line}`, fontSize: 12.5, color: p.success, fontWeight: 600 }}>
                      {t('✓ Accepted — see your booking', '✓ Aceptado — ve tu reservación')}
                    </div>
                  )}
                </div>
              );
      ...
      {counterTarget && (() => {
        ...
        return (
          <Modal
            ...
            eyebrow={t('COUNTER OFFER', 'CONTRAOFERTA')}
            title={lang === 'es' ? `Propón un precio a ${counterTarget.businesses?.name || 'este salón'}` : `Propose a price to ${counterTarget.businesses?.name || 'this salon'}`}
            width={460}
            footer={(
              <>
                <button onClick={() => setCounterTarget(null)} disabled={submittingCounter} style={{ background: 'transparent', border: `0.5px solid ${p.line}`, padding: '11px 18px', borderRadius: 99, fontSize: 13, fontWeight: 600, color: p.ink, cursor: submittingCounter ? 'wait' : 'pointer', fontFamily: 'inherit' }}>{t('Cancel', 'Cancelar')}</button>
                <button onClick={submitCounter} disabled={submittingCounter || !validPrice} style={{ background: p.ink, color: p.bg, border: 0, padding: '11px 22px', borderRadius: 99, fontSize: 13, fontWeight: 600, cursor: (submittingCounter || !validPrice) ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: (submittingCounter || !validPrice) ? 0.6 : 1 }}>
                  {submittingCounter ? t('Sending…', 'Enviando…') : (lang === 'es' ? `Enviar · $${price || 0}` : `Send · $${price || 0}`)}
                </button>
              </>
            )}
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ fontSize: 13, color: p.inkSoft, lineHeight: 1.5 }}>
                {lang === 'es' ? (
                  <>Su precio es <strong style={{ color: p.ink }}>${ask}</strong>. Propón lo que estarías dispuesta a pagar — pueden aceptarlo, rechazarlo (queda su precio original) o regresar con una nueva cotización.</>
                ) : (
              ...
              <div>
                <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.14em', color: p.inkMuted, marginBottom: 8 }}>{t('YOUR COUNTER', 'TU CONTRAOFERTA')}</div>
              ...
              <div>
                <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.14em', color: p.inkMuted, marginBottom: 8 }}>{t('NOTE TO SALON · OPTIONAL', 'NOTA PARA EL SALÓN · OPCIONAL')}</div>
                <textarea
                  value={counterDraft.message}
                  onChange={e => setCounterDraft({ ...counterDraft, message: e.target.value })}
                  rows={3}
                  placeholder={t('e.g. Repeat client, can come on a slow day', 'p.ej. Cliente frecuente, puedo ir un día lento')}
