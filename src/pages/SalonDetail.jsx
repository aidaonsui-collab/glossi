  ...
  if (loadingBiz) {
    return (
      <CustomerLayout mobileTitle="Salon">
        <div style={{ padding: '40px 30px', color: p.inkMuted, fontSize: 14 }}>Loading…</div>
      </CustomerLayout>
    );
  }
    ...
    return (
      <CustomerLayout mobileTitle="Salon">
        <div style={{ padding: '80px 32px', textAlign: 'center' }}>
          ...
          <button onClick={() => navigate('/explore')} style={{ marginTop: 24, padding: '12px 22px', borderRadius: 99, background: p.ink, color: p.bg, border: 0, fontSize: 14, cursor: 'pointer' }}>Back to explore</button>
        </div>
      </CustomerLayout>
    );
  }
  ...
  return (
    <CustomerLayout mobileTitle={biz.name}>
      <div style={{ maxWidth: 760, padding: isPhone ? '20px 18px 60px' : '34px 40px 60px' }}>
        <button onClick={() => navigate(-1)} style={{ background: 'transparent', border: 0, padding: 0, fontSize: 12, color: p.inkMuted, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>← Back</button>
        ...
        <div style={{ marginTop: 18, display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          <div style={{ flex: '1 1 240px', minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              {biz.verified && <span style={{ fontSize: 10, color: p.accent, fontWeight: 700, letterSpacing: '0.12em' }}>VERIFIED</span>}
            </div>
        ...
        <div style={{ marginTop: 36 }}>
          ...
          <div style={{ marginTop: 14 }}>
            {reviewsLoading ? (
              <div style={{ padding: 22, color: p.inkMuted, fontSize: 13 }}>Loading reviews…</div>
            ) : reviews.length === 0 ? (
              <div style={{ padding: 22, background: p.surface, borderRadius: 14, border: `0.5px dashed ${p.line}`, color: p.inkSoft, fontSize: 13.5, lineHeight: 1.55 }}>
                No reviews yet. Be the first — book and leave one after.
              </div>
            ) : (
                  ...
                  <div key={r.review_id} style={{ padding: '16px 18px', background: p.surface, borderRadius: 14, border: `0.5px solid ${p.line}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: p.inkMuted, marginTop: 2 }}>{new Date(r.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
                      </div>
                    ...
                    {r.salon_reply && (
                      <div style={{ marginTop: 10, padding: '10px 12px', background: p.bg, borderRadius: 10, borderLeft: `3px solid ${p.accent}` }}>
                        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', color: p.accent }}>SALON REPLY</div>
